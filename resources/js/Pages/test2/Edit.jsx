...
export default function EditTransaction({ transaction, incomes: initialIncomes, expenses: initialExpenses, categories }) {
    console.log("Props received by EditTransaction:", { transaction, initialIncomes, initialExpenses, categories });
    ...
    const { data, setData, put, processing } = useForm({
        ...
        date_paid: transaction.date_paid ? new Date(transaction.date_paid).toISOString().slice(0, 10): '',
        incomes: initialIncomes && initialIncomes.length > 0
                 ? initialIncomes.map(item => ({
                     id: item.id,
                     name: item.name,
                     nominal: item.value_idr, 
                 }))
                 : [{ id: null, name: '', nominal: '' }],
        expenses: initialExpenses && initialExpenses.length > 0
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route('test2.update', transaction.id), {
            onSuccess: () => {
            },
            onError: (formErrors) => {
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
                <h1 className="text-2xl font-bold mb-6 text-gray-800">EDIT DATA TRANSAKSI</h1>
                ...
                <form onSubmit={submit}>
                    ...
                    <div className="mb-8 p-4 border border-gray-200 rounded-md">
                        ...
                        {data.incomes.map((income, index) => (
                            <div key={income.id || `new-income-${index}`} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3 items-end">
                                {/* Hidden input for existing ID */}
                                {income.id && <input type="hidden" name={`incomes.${index}.id`} value={income.id} />}
                                <div>
                    ...
                    <div className="mb-8 p-4 border border-gray-200 rounded-md">
                        ...
                        {data.expenses.map((expense, index) => (
                            <div key={expense.id || `new-expense-${index}`} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3 items-end">
                                {/* Hidden input for existing ID */}
                                {expense.id && <input type="hidden" name={`expenses.${index}.id`} value={expense.id} />}
                                <div>
